// audio.js: overlap-safe audio for KOMMANDO
(() => {
  'use strict';
  ...
  // Shared defaults for file-backed sounds (override via configureFiles)
  const DEFAULT_FILE_MAP = {
    ...
    shoot:       'audio/sfx/shot.mp3',
    // Use procedural footsteps by default; provide an array to override with custom files
    footsteps: null,
  };
    ...
    configureFiles(map){
      ...
      if (src){
        for (const [k,v] of Object.entries(src)){
          if (v === null || v === false) { delete this.files[k]; continue; }
          if (typeof v === 'string' && v.trim()) { this.files[k] = v.trim(); continue; }
          if (Array.isArray(v) && v.every(x => typeof x === 'string' && x.trim())) { this.files[k] = v.slice(); continue; }
        }
      }
    },
  ...
  // Surface coloration for footsteps (used by playFootstep)
  const SURFACE_TWEAKS = {
    stone: { detuneCents: [-40, 40], gain: [0.75, 0.95] },
    dirt:  { detuneCents: [-20, 20], gain: [0.80, 1.00] },
    metal: { detuneCents: [ 20,  90], gain: [0.70, 0.90] },
    water: { detuneCents: [-80,-10], gain: [0.65, 0.85] },
    default:{ detuneCents: [-35, 35], gain: [0.78, 0.96] },
  };
  ...
  // Procedural footstep fallback: short thump + scrape
  function buildFootstep(d, sr, ch){
    const dur = d.length / sr;
    addBlip(d, sr, {
      start: 0,
      dur: Math.min(0.09, dur * 0.6),
      freq: (t)=> 140 - 60*t,
      amp: 0.55,
      envPower: 2.2,
      harmonics:[0.18]
    ...
    for(let i=0;i<d.length;i++){
      const t = i/sr;
      const n = (rnd(i, 31+ch) - 0.5) * 0.22;
      const env = Math.max(0, 1 - t / Math.min(0.12, dur));
      d[i] += n * env * 0.55;
    }
  }

  function buildShoot(d, sr, ch){
    addBlip(d, sr, { start: 0, dur: 0.055, freq: (t) => 380 - 240 * t, amp: 0.48, envPower: 2.6, harmonics: [0.2] });
    for (let i = 0; i < d.length; i++) {
      const t = i / sr;
      const env = Math.max(0, 1 - t / 0.07);
      d[i] += (rnd(i, 19 + ch) - 0.5) * 0.32 * env;
    }
  }
  ...
  const SOUND_SPECS = {
    ...
    footsteps:  { duration: 0.18, edgeFade: 0.005, target: 0.90, builder: buildFootstep },
    shoot:      { duration: 0.10, edgeFade: 0.005, target: 0.90, builder: buildShoot },
  };
  ...
  const A = {
    ...
    // footsteps state
    lastStepAt: 0,
    stepCooldownBase: 0.12, // seconds at normal pace
    _stepSide: 0,           // 0 = left, 1 = right (for subtle alternation)
    ...
    configureFiles(map){
      ...
      // Start with defaults
      this.files = {...DEFAULT_FILE_MAP};
      // Prefer explicit object param; otherwise, look for global window.KomAudioFiles
      const globalMap = (typeof window !== 'undefined' && window.KomAudioFiles && typeof window.KomAudioFiles === 'object')
        ? window.KomAudioFiles
        : null;
      const src = (map && typeof map === 'object') ? map : globalMap;
      ...
      // Clear decoded file buffers so changes take effect immediately
    ...
    obtainBuffer(name){
      this._ensureStores();
      let filePath = this.files[name];

      // If we have a group (array), pick one each call for variety
      if (Array.isArray(filePath)) {
        const chosen = filePath[(Math.random()*filePath.length)|0];
        filePath = chosen;
      }
      ...
      if (filePath){
        ...
        const loading = fetch(filePath)
          ...
          .catch(err=>{
            ...
            // If a group failed, remove that one path; if all fail, fall back to procedural builder
            if (Array.isArray(this.files[name])) {
              this.files[name] = this.files[name].filter(p => p !== filePath);
              if (this.files[name].length === 0) delete this.files[name];
            } else {
    ...
    playShoot(){ this.play('shoot', { gain: 0.72 }); },

    setMuted(on){
      this.muted = !!on;
      try { localStorage.setItem('kommando-muted', this.muted ? '1' : '0'); } catch (_) {}
      if (this.muted) {
        try { this.stopBackground({ fade: true }); } catch (_) {}
        try { this.stopOneShots({ fade: true }); } catch (_) {}
      } else if (this.userActivated) {
        try { this.startBackground(this._autoBgOpts || { gain: 0.62, fadeIn: 0.05 }); } catch (_) {}
      }
      return this.muted;
    },
    ...
    startBackground(opts = {}){
      ...
      if (notReady){
        // Only stash options; do not touch `pendingLoops` here. A deferred "pending" with no
        // running loop made the first post-gesture startBackground() a no-op until hardStopAll
        // cleared state on the next level (soundtrack missing on level 1).
        this._autoBgOpts = opts || this._autoBgOpts || {};
        return;
      }
    ...
    // ---------- Walking / footstep ----------
    playFootstep({ surface='default', pace=1, alternate=true } = {}){
      const ctx = this.ensureContext(true);
      if(!ctx) return;

      const now = ctx.currentTime || 0;
      const cooldown = Math.max(0.06, this.stepCooldownBase / Math.max(0.4, pace)); // faster pace => shorter cooldown
      if (now - this.lastStepAt < cooldown) return;
      this.lastStepAt = now;

      const tw = SURFACE_TWEAKS[surface] || SURFACE_TWEAKS.default;
      const r = (a,b)=> a + Math.random()*(b-a);

      const pending = this.obtainBuffer('footsteps');
      const asPromise = (pending && typeof pending.then === 'function') ? pending : Promise.resolve(pending);
      ...
      asPromise.then((buffer)=>{
        ...
        // detune in cents (fallback to playbackRate if not supported)
        const cents = r(tw.detuneCents[0], tw.detuneCents[1]) + (alternate ? (this._stepSide ? 12 : -12) : 0);
        if ('detune' in src) {
        ...
        const baseGain = r(tw.gain[0], tw.gain[1]);

        // tiny fade to avoid clicks
        const t0 = now + 0.001;
        const fadeIn = 0.004, fadeOut = 0.06;
        g.gain.setValueAtTime(0, t0);
        g.gain.linearRampToValueAtTime(baseGain, t0 + fadeIn);
        const dur = buffer.duration || 0.18;
        g.gain.setValueAtTime(baseGain, t0 + Math.max(0.02, dur - fadeOut));
        g.gain.linearRampToValueAtTime(0, t0 + dur);

        // stereo panner (optional)
        let panner = null;
        if (alternate && 'createStereoPanner' in ctx) {
          panner = ctx.createStereoPanner();
          panner.pan.value = this._stepSide ? 0.12 : -0.12;
        ...
        src.onended = () => {
          ...
          try { if (panner) panner.disconnect(); } catch(_){}
        };
        if (alternate) this._stepSide ^= 1;
      });
    },
  };
  ...
  try { A.muted = localStorage.getItem('kommando-muted') === '1'; } catch(e){}
